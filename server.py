...
VIDEO_EXTENSIONS = {".mp4", ".m4v", ".webm", ".mov", ".ts"}
...
def index():
    # Read the current Google Drive library so the page always reflects the latest structure.
    library = build_library()
    # Build the standalone-movie rows with searchable data attributes.
    movie_items = "".join(
        '<li class="movie-item" data-search="%s"><a href="/watch/%s">%s</a></li>'
        % (
    ...
    # Build collapsible series sections; every series starts closed to keep the page compact.
    series_items = "".join(
        '<li class="series-item" data-series-search="%s">'
        '<details class="series-details" data-series-key="%s">'
        '<summary><span class="series-name">%s</span><span class="episode-count">%d tập</span></summary>'
        '<ul class="episode-list">%s</ul>'
        '</details></li>'
        % (
            safe_text(show["name"]).lower(),
            show["key"],
            safe_text(show["name"]),
            len(show["episodes"]),
            "".join(
                '<li class="episode-item" data-search="%s">'
                '<a href="/watch/%s">%s</a></li>'
                % (
                    safe_text(episode["name"]).lower(),
                    episode["key"],
                    safe_text(episode["name"]),
                )
                for episode in show["episodes"]
    ...
    # Return a responsive library page with search and persistent collapsible series folders.
    return HTMLResponse(
        """<!doctype html>
<html lang="vi">
<head>
...
body { margin: 0; padding: 24px 16px; background: #101114; color: #f2f2f2; font-family: Arial, sans-serif; }
...
.search { flex: 1; min-width: 0; padding: 12px 14px; border: 1px solid #333; border-radius: 9px; background: #1a1c21; color: #fff; font-size: 16px; outline: none; }
...
.series-item { margin-bottom: 8px; border: 1px solid #292c33; border-radius: 10px; overflow: hidden; background: #17191e; }
.series-details summary { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 14px 15px; cursor: pointer; list-style: none; user-select: none; }
.series-details summary::-webkit-details-marker { display: none; }
.series-details summary::before { content: "▶"; font-size: 11px; width: 14px; transition: transform .15s ease; }
.series-details[open] summary::before { transform: rotate(90deg); }
.series-name { flex: 1; font-weight: 600; }
.episode-count { color: #8f949d; font-size: 13px; white-space: nowrap; }
.episode-list { padding: 0 15px 8px 43px; background: #13151a; }
.no-results { color: #8f949d; padding: 12px 2px; }
.footer { margin-top: 28px; font-size: 13px; color: #777; }
@media (max-width: 600px) {
    body { padding: 18px 12px; }
    .toolbar { flex-direction: column; }
    .refresh { width: 100%; }
    .episode-list { padding-left: 30px; }
}
</style>
</head>
<body>
...
<h2>Phim lẻ</h2>
<p class="section-note">Các video nằm trực tiếp trong thư mục Videos Streaming.</p>
<ul id="movies">
    __MOVIES__
</ul>

<h2>Phim bộ</h2>
<p class="section-note">Bấm vào tên phim để mở hoặc đóng danh sách tập.</p>
<ul id="series">
    __SERIES__
</ul>

<div id="no-results" class="no-results" hidden>Không tìm thấy phim hoặc tập phù hợp.</div>
<div class="footer"><a href="/health">Server health</a></div>
</div>

<script>
// Keep the search box and folder state entirely in this browser.
...
// Get all series rows.
const seriesItems = Array.from(document.querySelectorAll(".series-item"));
// Get every collapsible series folder.
const seriesDetails = Array.from(document.querySelectorAll(".series-details"));
// Get the empty-search result message.
const noResults = document.getElementById("no-results");
// Use one localStorage key for all folder open/closed states.
const folderStateKey = "drive_stream_series_open";

// Read the previously saved folder state without failing if storage is unavailable.
function loadFolderState() {
    try {
        return JSON.parse(localStorage.getItem(folderStateKey) || "{}");
    } catch (error) {
        return {};
    }
}

// Save the current folder state so a page refresh keeps the same open folders.
function saveFolderState() {
    const state = {};
    seriesDetails.forEach(function(details) {
        state[details.dataset.seriesKey] = details.open;
    });
    try {
...
// Restore folders that were open during the previous visit.
const savedFolderState = loadFolderState();
seriesDetails.forEach(function(details) {
...
// Filter movies, series and episodes whenever the user types in the search box.
function applySearch() {
    const query = normalizeText(search.value.trim());
    let visibleCount = 0;

    // Show or hide each standalone movie according to its filename.
    movieItems.forEach(function(item) {
        const match = !query || normalizeText(item.dataset.search || "").includes(query);
        item.hidden = !match;
        if (match) visibleCount += 1;
    });

    // Show a series when its name or at least one episode matches the search.
    seriesItems.forEach(function(seriesItem) {
        const details = seriesItem.querySelector(".series-details");
        const seriesName = normalizeText(seriesItem.dataset.seriesSearch || "");
        const episodes = Array.from(seriesItem.querySelectorAll(".episode-item"));
        const seriesMatches = !query || seriesName.includes(query);
        let episodeMatches = false;

        episodes.forEach(function(episode) {
            const match = !query || seriesMatches || normalizeText(episode.dataset.search || "").includes(query);
            episode.hidden = !match;
            if (match) episodeMatches = true;
        });

        const showSeries = !query || seriesMatches || episodeMatches;
        seriesItem.hidden = !showSeries;
        if (showSeries) visibleCount += 1;

        // During a search, automatically open matching folders so the matching episodes are visible.
        if (query && showSeries) {
            details.open = true;
        }
    });

    // Tell the user when nothing matches the current search.
    noResults.hidden = visibleCount !== 0;
}

// Apply filtering as soon as the user changes the search field.
search.addEventListener("input", applySearch);

// Refresh the Google Drive library and reload the page with the new contents.
refreshButton.addEventListener("click", async function() {
    ...
    try {
        ...
        refreshButton.textContent = "Lỗi - thử lại";
    }
});

// Run the initial filter once so the page state is consistent immediately.
applySearch();
</script>
</body>
</html>"""
        .replace("__MOVIES__", movie_items or '<li class="no-results">Chưa có phim lẻ.</li>')
        .replace("__SERIES__", series_items or '<li class="no-results">Chưa có phim bộ.</li>')
    )
